...
void mat_rand(Mat m, int min, int max){
...
void nn_rand(NN nn, int min, int max){
...
float slope(NN nn, Mat inputs, Mat expected_outputs, Mat current, int x, int y,float eps){
...
Mat nn_slope(NN nn, Mat inputs, Mat expected_outputs,Mat current,float eps){
    ...
    for(int x = 0; x < current.rows; ++x){
        for(int y = 0; y < current.cols; ++y){
            mat_at(gradient,x,y) = slope(nn,inputs,expected_outputs,current,x,y,eps);
        }
    }
    return gradient;
}

void nn_gradient(NN nn, NN gradient, Mat inputs, Mat expected_outputs,float eps){
    ...
    for(int i = 0; i < nn.count; ++i){
        ...
        Mat ws = nn_slope(nn,inputs,expected_outputs,nn.w[i],eps);
        mat_copy(gradient.w[i],ws);
        Mat bs = nn_slope(nn,inputs,expected_outputs,nn.b[i],eps);
        mat_copy(gradient.b[i],bs);
    }
}

void nn_learn(NN nn,NN gradient, Mat inputs, Mat expected_outputs,float eps, float rate){
    //Modifying weights and biases to reduce cost
    nn_gradient(nn,gradient,inputs,expected_outputs,eps);
    for(int i = 0; i < nn.count; ++i){
